...
- `[done] 2026-04-16 — Custom attributes i Browse/Edit view` — viser Owner, Location, AuthzVlan kolonner i browse-tabellen med dropdown-redigering. Henter fuld endpoint-detalje inkl. custom attrs fra ISE. Nye routes: `GET /api/endpoints/details`, `GET /api/endpoints/{id}`. Lag: backend (api, services, schemas), frontend (api, browse view).
